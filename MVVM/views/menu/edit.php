...
$menu = $menuViewModel->getMenuById($menuId);

if (!$menu) {
    header('Location: index.php?page=menu&error=Menu tidak ditemukan');
    exit;
}

// Handle form submission
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $_POST['id'] = $menuId; // Ensure ID is set for update
    if ($menuViewModel->saveMenu($_POST)) {
        header('Location: index.php?page=menu&success=Menu berhasil diperbarui');
        exit;
    } else {
    ...
    <div class="card shadow">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">
                <i class="fas fa-edit"></i> Form Edit Menu
            </h6>
        </div>
        <div class="card-body">
